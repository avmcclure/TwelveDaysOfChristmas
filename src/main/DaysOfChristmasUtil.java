...
public final class DaysOfChristmasUtil {
	...
	private static final String DAY_RANGE = "Day must be greater than " + (MIN_DAY - 1) + " and less than "
			+ (MAX_DAY + 1) + ".";
	...
	public static List<Present> getIndividualPresentsForDay(int selectedDay) {
		...
		int index = 0;
		for (int day = selectedDay; day > 0; day--, index++) {
	...
	private static Present getPresentForDay(int day, int index) {
		PresentEnumerable presentType = PRESENTS_BY_DAY[index];
		int count = day * (index + 1);
		Present present = new Present(presentType, count);
		return present;
	}
